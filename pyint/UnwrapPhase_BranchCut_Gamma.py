...
#            Date  :   February, 2017                                             #
#                                                                                 #
#         Unwrap interferograms based on Branch-Cut Method                        #
#                                                                                 #
###################################################################################
#'''
import numpy as np
...
******************************************************************************************************
   ...
   usage:
            ...
            UnwrapPhase_BranchCut_Gamma.py igramDir
      
      e.g.  UnwrapPhase_BranchCut_Gamma.py IFG_PacayaT163TsxHhA_131021-131101_0011_0007
      e.g.  UnwrapPhase_BranchCut_Gamma.py MAI_PacayaT163TsxHhA_131021-131101_0011_0007
      e.g.  UnwrapPhase_BranchCut_Gamma.py RSI_PacayaT163TsxHhA_131021-131101_0011_0007          
...
def main(argv):
    ...
    if 'branch_length' in templateContents: mbl = templateContents['branch_length']
    else: mbl = '32'
...
#  Definition of file
    ...
    for i in range(len(Suffix)):          
        ...
        UNWFLAGlks  = workDir + '/' + Mdate + '-' + Sdate + '_' + rlks + 'rlks' + Suffix[i] + '.unw_flag'
...
#########################################################    
        if 'Subset_Rdc' in templateContents: 
            ...
            if not XX[3] == '-':
                ...
                ERL = str(int(NR) + int(SRU))
                SAU= XX[2]
                NA = str(int(XX[3])-int(XX[2]))
                EAL = str(int(NA) + int(SAU))
            else:
                SRU = '-'
                NR = '-'
                SAU = '-'
        ...
        if os.path.isfile(UNWFLAGlks):
            os.remove(UNWFLAGlks)
        ...
        call_str = '$GAMMA_BIN/corr_flag ' + CORDIFFlks + ' ' + UNWFLAGlks + ' ' + nWidth + ' ' + unwrappedThreshold + ' ' + SRU + ' ' + ERL + ' ' + SAU + ' ' + EAL
        os.system(call_str)
        
        call_str = '$GAMMA_BIN/residue_cc ' + DIFFINTFILTlks + ' ' + UNWFLAGlks + ' ' + nWidth + ' ' + SRU + ' ' + ERL + ' ' + SAU + ' ' + EAL
        os.system(call_str)
        
        call_str = '$GAMMA_BIN/tree_cc ' +  UNWFLAGlks + ' ' + nWidth + ' ' + mbl + ' ' + SRU + ' ' + ERL + ' ' + SAU + ' ' + EAL
        os.system(call_str)
       
         
        call_str = '$GAMMA_BIN/grasses ' + DIFFINTFILTlks + ' ' + UNWFLAGlks  + ' ' + UNWlks + ' ' + nWidth + ' ' + SRU + ' ' + ERL + ' ' + SAU + ' ' + EAL + ' ' + Ref_Range + ' ' + Ref_Azimuth
        os.system(call_str)
        ...
        if flatteningUnwrap == 'Y':
            ...
            call_str = '$GAMMA_BIN/quad_fit ' + UNWlks + ' ' + DIFFpar + ' 32 32 ' + CORMASK + '_mask.bmp ' + QUADFIT + ' 0'
            os.system(call_str)
    ...
    print("Uwrapping interferometric phase based on Branch-cut is done!")
    sys.exit(1)
